  ...
  dots.forEach((dot, idx) => {
    ...
    dot.addEventListener('click', (e) => {
      ...
      if (clickedDotNum == activeDotNum) {
        ...
        let pixels = -(displayArea / 3 + 10) * clickedDotNum
  ...
  window.addEventListener('resize', (event) => {
    ...
    /* if (vw < 800) {
      ...
      Array.from(slides).forEach((slide) => {
        slide.style.flex = "0 0 calc(100%/1)"
      })
    }
    if (vw >= 800 && vw < 1020) {
      console.log("Milieu", vw)
      Array.from(slides).forEach((slide) => {
        slide.style.flex = "0 0 calc(100%/2)"
      })
    }
    if (vw >= 1020) {
      Array.from(slides).forEach((slide) => {
        slide.style.flex = "0 0 calc(100%/3)"
      })
